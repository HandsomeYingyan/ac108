...
int ac10x_voice_card_register_set_clock(int stream, int (*set_clock)(int)) {
	if (! _set_clock[stream]) {
		_set_clock[stream] = set_clock;
	}
	return 0;
}
EXPORT_SYMBOL(ac10x_voice_card_register_set_clock);
...
int ac101_codec_probe(struct snd_soc_codec *codec)
{
	...
	#if _MASTER_MULTI_CODEC == _MASTER_AC101
	ac10x_voice_card_register_set_clock(SNDRV_PCM_STREAM_PLAYBACK, ac101_set_clock);
	#endif
